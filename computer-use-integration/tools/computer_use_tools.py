...
class Tools:
    class Valves(BaseModel):
        ORCHESTRATOR_URL: str = Field(
            default=os.environ.get("COMPUTER_USE_ORCHESTRATOR_URL", "http://localhost:8081"),
            description="URL of the Computer Use MCP Server (upstream). Set COMPUTER_USE_ORCHESTRATOR_URL env var or update here. Points to ur Coolify-deployed MCP server!"
        )
        MCP_API_KEY: str = Field(
            default=os.environ.get("COMPUTER_USE_MCP_API_KEY", ""),
            description="Bearer token for MCP server authentication (set via COMPUTER_USE_MCP_API_KEY env var)"
        )
